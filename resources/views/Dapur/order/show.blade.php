<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('Detail Order Dapur') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg p-8">
                <h3 class="text-2xl font-bold mb-6 text-yellow-700 dark:text-yellow-400 flex items-center gap-2">
                ...
                <div class="mb-6">
                    <label for="status_dapur" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
                        Status Pengiriman Dapur
                    </label>
                    <div class="flex flex-col md:flex-row items-start md:items-center gap-4">
                        <select id="status_dapur" class="w-full md:w-1/2 rounded border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 shadow-sm"
                            data-order-id="{{ $order->id }}">
                            <option value="Belum Diproses" {{ $order->status_dapur == 'Belum Diproses' ? 'selected' : '' }}>Belum Diproses</option>
                            <option value="Diproses" {{ $order->status_dapur == 'Diproses' ? 'selected' : '' }}>Diproses</option>
                            <option value="Terkirim" {{ $order->status_dapur == 'Terkirim' ? 'selected' : '' }}>Terkirim</option>
                        </select>
                        <button id="btn-update-status" type="button"
                            class="inline-block bg-yellow-600 hover:bg-yellow-700 text-white font-semibold px-6 py-2 rounded shadow transition">
                            Update Status
                        </button>
                    </div>
    ...
    <script>
        document.getElementById('btn-update-status').addEventListener('click', function() {
            const select = document.getElementById('status_dapur');
            ...
            fetch(`/dapur/orders/${orderId}/update-status`, {
                    method: 'POST',
                    headers: {
